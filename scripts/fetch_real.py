...
Output (all committed; that makes the demo reproducible offline):
  data/real/dataset.json       HydraDB-ready graph (same supply-chain schema as data/github)
...
APPS: list[tuple[str, dict[str, str]]] = [
    (
        "api-gateway",
        {"express": "4.17.1", "ws": "6.2.2", "minimist": "1.2.5", "axios": "0.21.1"},
    ),
    ("web-console", {"lodash": "4.17.20", "marked": "4.0.10", "debug": "4.3.1"}),
    (
        "data-ingest",
        {"got": "11.8.5", "node-fetch": "2.6.1", "qs": "6.10.1", "minimist": "0.0.8"},
    ),
    ("auth-api", {"jsonwebtoken": "8.5.1", "express": "4.17.1", "ws": "7.4.5"}),
    (
        "metrics-exporter",
        {"axios": "0.21.1", "ws": "6.2.2", "marked": "4.0.10", "lodash": "4.17.20"},
    ),
]
...
def http_json(
    url: str, method: str = "GET", body: dict | None = None, timeout: int = 60
) -> dict:
    ctx = None
    try:
    ...
    req = urllib.request.Request(
        url, method=method, headers={"User-Agent": UA, "Accept": "application/json"}
    )
...
def _parse(v: str) -> tuple:
    ...
    return (
        (int(m.group(1)), int(m.group(2) or 0), int(m.group(3) or 0))
        if m
        else (0, 0, 0)
    )
...
def lockfile_pins(lf_path: Path) -> list[tuple[str, str]]:
    ...
    return [
        (k, meta.get("version", ""))
        for k, meta in data.get("dependencies", {}).items()
        if meta.get("version")
    ]
...
def fetch_osv(names: list[str]) -> dict[str, list[dict]]:
    ...
    if known_cache.exists():
        cached = {
            rec["package"]: rec["vulns"] for rec in json.loads(known_cache.read_text())
        }
...
def main() -> int:
    ...
    for app, deps in APPS:
        ...
        lock_data[app] = {
            ...
            "pins": pins,
            "lockfile": lf.read_text(),
            "npm": 0,
        }
    lock_data["api-gateway"]["npm"] = (
        1  # placeholder; real versions recorded in node/npm node
    )
    ...
    for name in sorted(universe):
        ...
        times = doc.get("time", {})
        mnt = [
            (m.get("name") or m.get("email", "?")) for m in doc.get("maintainers", [])
        ]
        for v in sorted(set(keeps)):
            ...
            props = {
                "name": name,
                "version": v,
                "published_at": _iso_to_epoch(times.get(v))
                or _iso_to_epoch(times.get("created")),
                "deprecated": bool(meta.get("deprecated")),
                "maintainers": mnt,
            }
            nodes.append(
                {"label": "PackageVersion", "id": ver_id(name, v), "properties": props}
            )
    ...
    for app, meta in lock_data.items():
        ...
        group_edges(
            "USES_LOCKFILE",
            {
                ...
                "target": lock_id,
                "properties": {
                    "since": meta["fetched_at"],
                    "valid_from": meta["fetched_at"],
                    "valid_until": 4102444800,
                },
            },
        )
        for name, ver in meta["pins"]:
            ...
            group_edges(
                "RESOLVES_TO",
                {
                    ...
                    "source": lock_id,
                    "target": pkg_versions[name][ver],
                    "properties": {
                        "at": meta["fetched_at"],
                        "was_resolved_while_live": False,
                        "valid_from": meta["fetched_at"],
                        "valid_until": 4102444800,
                    },
                },
    ...
    for n in list(nodes):
        for m in n["properties"].get("maintainers", []) or []:
            ...
            nodes.append(
                {"label": "Developer", "id": dev_ids[m], "properties": {"handle": m}}
            )
    for n in nodes:
    ...
    for name, vulns in osv.items():
        for v in vulns:
            adv_id = v.get("id", "?")
            cve = next(
                (a for a in v.get("aliases", []) if a.startswith("CVE-")), adv_id
            )
            sev = 0
            for s in v.get("severity", []):
                m = re.match(
                    r"CVSS:3\.[01]/AV:[NAL]/AC:[LH]/PR:[NALH]/UI:[NR]/S:[CU]/C:([LH])/I:([LH])/A:([LH])",
                    s.get("score", ""),
                )
                if m:
            ...
            if not sev:
                ...
                sev = {"LOW": 1, "MODERATE": 4, "HIGH": 7, "CRITICAL": 9}.get(
                    str(ds.get("severity", "")), 0
                )
            for aff in v.get("affected", []):
                ...
                for ver in list(pkg_versions[pkg]):
                    ...
                    if better and (
                        nid not in fixver or _vercmp(better[0], fixver[nid]) < 0
                    ):
                        fixver[nid] = better[0]
    ...
    print(
        f"packages={len(pkg_versions)} versions={len(nodes_for)} locks={ {k: len(v['pins']) for k, v in lock_data.items()} }"
    )
    return 0
...
def _affected_expr(aff: dict) -> str:
    ...
    for rng in aff.get("ranges", []):
        ...
        if low or high:
            exprs.append(
                f"{'>=' + low if low else ''} {'<' + high if high else ''}".strip()
            )
